...
class CompleteTrainingRequest(BaseModel):
    ...
    learning_rate: float = Field(
        default=5e-4, description="Base learning rate for optimizer"
    )
    b_up_factor: float = Field(
        default=3.0, description="Learning rate multiplier for lora_B parameters"
    )
    batch_size: int = Field(default=4, description="Batch size per GPU")
    gradient_accumulation_steps: int = Field(
        default=1, description="Number of steps to accumulate gradients"
    )
...
class TrainingRequest(BaseModel):
    ...
    max_train_steps: int = Field(default=250, description="Number of training steps")
    batch_size: int = Field(default=4, description="Batch size per GPU")
    gradient_accumulation_steps: int = Field(
        default=1, description="Number of steps to accumulate gradients"
    )
    guidance_scale: float = Field(
        default=1.0, description="Guidance scale for training"
    )
    use_masks: bool = Field(
        default=True, description="Whether to use masks (for face-focused training)"
    )
    ...
    lr_warmup_steps: int = Field(
        default=0, description="Number of warmup steps for learning rate"
    )
    seed: int = Field(default=42, description="Random seed for reproducibility")
...
class TrainingResponse(BaseModel):
    ...
    checkpoint: File = Field(description="Trained LoRA checkpoint file")
    final_loss: float = Field(description="Final average training loss")
    num_steps: int = Field(description="Total number of training steps completed")
...
class FluxLoRATrainingApp(fal.App):
    ...
    async def train_from_preprocessed_stream(
        ...
        async def generate_stream():
            with tempfile.TemporaryDirectory() as temp_dir:
                print(
                    f"Downloading preprocessed data from {request.training_data_url.url}"
                )
                training_data_path = str(
                    download_file(request.training_data_url.url, target_dir=temp_dir)
                )
