...
class FluxPreprocessorWorker(DistributedWorker):
    ...
    def setup(
        self,
        model_path: str = "/data/flux_weights",
        **kwargs: Any
    ) -> None:
        """
    ...
    def generate_captions_parallel(
        ...
        self.rank_print(f"Generating captions for {len(local_image_paths)} images (indices {start_idx}:{end_idx})...")
    ...
    def encode_images_parallel(
        ...
        self.rank_print(f"Encoding {len(local_image_paths)} images (indices {start_idx}:{end_idx})...")
        ...
        if latents_list:
            ...
            local_latents = torch.empty(0, 16, resolution // 8, resolution // 8, device=self.device)
    ...
    def encode_text_parallel(
        ...
        end_idx = start_idx + captions_per_rank if self.rank < self.world_size - 1 else len(captions)
    ...
    def gather_tensors(self, local_tensor: torch.Tensor) -> torch.Tensor:
        ...
        all_shapes = [None] * self.world_size
        ...
        max_size = max(shape[0] for shape in all_shapes)
        ...
        if local_tensor.shape[0] < max_size:
            ...
            padding = torch.zeros(padding_shape, dtype=local_tensor.dtype, device=self.device)
        ...
        gathered_list = [torch.zeros_like(local_tensor_padded, device=self.device) for _ in range(self.world_size)]
        ...
        if self.rank == 0:
            ...
            result_list = []
            for gathered, shape in zip(gathered_list, all_shapes):
                actual_size = shape[0]
                if actual_size > 0:
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        if dist.is_initialized():
            self.rank_print(f"[{time_module.time():.2f}] torch.distributed IS initialized, backend={dist.get_backend()}, rank={dist.get_rank()}, world_size={dist.get_world_size()}")
        else:
            self.rank_print(f"[{time_module.time():.2f}] WARNING: torch.distributed is NOT initialized!")
        
        self.rank_print(f"[{time_module.time():.2f}] Starting preprocessing: request_id={request_id}, trigger_word='{trigger_word}', resolution={resolution}")
        ...
        self.rank_print(f"[{time_module.time():.2f}] Extract dir will be: {extract_dir}")
        
        # Rank 0 extracts the ZIP and gets image paths
        if self.rank == 0:
            self.rank_print(f"[{time_module.time():.2f}] Rank 0: Creating directory {extract_dir}")
            extract_dir.mkdir(exist_ok=True, parents=True)
            self.rank_print(f"[{time_module.time():.2f}] Rank 0: Directory created")
            
            self.rank_print(f"[{time_module.time():.2f}] Rank 0: Extracting images from {images_zip_url}")
            image_paths = self.extract_zip(images_zip_url, str(extract_dir))
            self.rank_print(f"[{time_module.time():.2f}] Rank 0: Found {len(image_paths)} images")
            self.rank_print(f"[{time_module.time():.2f}] Rank 0: Image paths: {[str(p) for p in image_paths[:3]]}...")
        else:
            image_paths = None
        
        # Step 2: Broadcast image_paths to all ranks (no filesystem handoff!)
        self.rank_print(f"[{time_module.time():.2f}] Rank {self.rank}: Broadcasting image paths...")
        ...
        self.rank_print(f"[{time_module.time():.2f}] Rank {self.rank}: Received {len(image_paths)} image paths")
        
        # Step 3: Generate captions in PARALLEL across all GPUs
        self.rank_print(f"[{time_module.time():.2f}] Rank {self.rank}: Starting parallel caption generation...")
        ...
        self.rank_print(f"[{time_module.time():.2f}] Rank {self.rank}: Broadcasting captions...")
        torch.cuda.set_device(self.device)
        captions_list = [captions]
        dist.broadcast_object_list(captions_list, src=0)
        captions = captions_list[0]
        self.rank_print(f"[{time_module.time():.2f}] Rank {self.rank}: Received {len(captions)} captions")
        ...
        text_embeddings, pooled_embeddings = self.encode_text_parallel(captions, batch_size=1)
        ...
        if self.rank == 0:
            ...
            output_path = Path("/data") / "flux_preprocessing" / f"preprocessed_{request_id}.pt"
